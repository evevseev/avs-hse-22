...
# Введение
## О программе
Программа изначально разрабатывалась с учетом всех требований включительно до оценки 9.
Поэтому, с самого начала, присутствуем разделение на модули, работа с файлами и прочий требуемый функционал.
После выполнения операции с массивом, в консоль будет выведено затраченное время в миллисекундах.

## Входные данные
Так как не было предъявлено дополнительных требований, ограничение на количество элементов: `1 <= n <= 300_000 (MAX_SIZE)`. Реализована проверка на соответствие данному диапазону.

Программа принимает на вход значения `int32`. При вводе значений массива, программа подразумевает, что на вход подаются корректные числовые значение `int32`. 
### Аргументы запуска 
|Аргумент|Описание|Пример|
|---|---|---|
|Без аргументов|Запуск программы в консольном режиме|`./a.out`|
|-file <входной файл> [выходной файл]|Передача и получение данных из программы с использованием файлов. Выходной файл может быть не указан, в таком случае результат выполнения будет выведен в консоль|`./a.out -file input.txt`, `./a.out -file input.txt output.txt`|
|-random <размер массива> <seed>|Запуск программы с генерацией случайных данных. Сгенерированный массив и результат выполнения выводятся в консоль|`./a.out -random 10 123`|
|-test <seed>|Стресс-тестирование программы. Генерируется массив случайных данных размером `MAX_SIZE`, после чего происходит его обработка. Данный процесс повторяется несколько тысяч раз. (данный каждый раз одни и те же, так как поражаются заданным <seed>, что позволяет сравнивать производительность при разных запусках) |`./a.out -test`|
### Представленные файлы
```
| /c-source - исходны код программы на C
| /tests
|   | /temp - папка с временными файлами генерируемые тестовой утилитой которые подаются на вход программе
|   | main.py - утилита тестирования
| /version-0 - ASM и исполняемый файл код полученный из c-source
| /version-1 - ASM код с комментариями и полученный исполняемый файл
| /version-2 - ASM код с преимущественным использованием регистров и исполняемый файл
| /with-optimization-flags
|   | /asms - ASM исходники полученные с использованием различных опций оптимизации
|   | /bins - исполняемые файлы полученные с использованием различных опций оптимизации
```
### [Функции и локальные переменные в ASM](#fdsfd)

## О тестировании
Для тестирования был написан специальный Python скрипт, который генерирует наборы случайных данных вида (input, correct_output) на которых прогоняется каждая программа.

Тестирование файлового и консольного режима происходит на одинаковых наборах.
...
Макросов в полученных ASM файлах не нашлось.
Удалил служебные метки GCC в конце файлов.
...
Корректность работы в файловом режиме была проверена с использованием автоматического и ручного тестирования программы.
...
|Task-2 (регистры)|26914|17320|1.120018|
...
Заметим, что `-O2` и `-O3` полностью идентичны. `-Ofast` не уступает `-O2` и `-О3`, однако имеет самый большой размер исполняемого файла. Худшие результаты времени у `c-source`, `Task-1`, `Task-1` (время исполнение одинаковое, что и следовало ожидать). Размер ASM `Task-1` и `Task-2` больше всех остальных из-за наличия комментариев в коде.

Если сравнивать `Task-2` и лучшую версию полученную компилятором `-O2`, у `Task-2` размер исполняемого файла меньше на 3%, время выполнения больше на 6%

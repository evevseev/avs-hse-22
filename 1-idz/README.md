...
Что сделал: проверил наличие макросов - их не оказалось
Удалил служебные метки GCC в конце, сложно их назвать макросами, но это точно лишний код и название файла из начала
метки в начале не удаляю, так как они полезные даже при ручной компиляции стоило выделить array на стеке хотя хз
...
# Mark 5
Все требуемые комментарии были написаны при выполнении пункта Mark 4. Каких-либо дополнительныз действий для выполнения этого Mark 5 не потребовалась
...
|--- |---|---|
|process_array|`i => r8`, `&array => rdi`, `size => esi`|Обращение к элементу массива теперь выполняется одной коммандой, локлаьных пересенных на стеке больше нет| 
|read_array_from_console|`&array => r15`, `max_size => r14`, `i => r13`|Обращение к элементу массива теперь выполняется одной коммандой, выделяется меньше памяти на стеке|
|print_array|`&array => r15`, `size => r14`, `i => r13`|Обращение к элементу массива теперь выполняется одной коммандой, локлаьных пересенных на стеке больше нет|
|read_array_from_file|`&filepath => rsi`, `&array => r15`, `max_size => r14`|Выделяется в два раза меньше памяти|
|save_array_to_file|`&filepath => rdx`, `&array => r15`, `size => r14`|Выделяется в три раза меньше памяти|
|random_fill_array|`&array => r15`, `size => r14`, `seed => edx`||

можно еще было адрес сохранять но на скорость это не влияло
...
r12, 
r13    consoleio i /
r14, - array size random    / array size fileio.s print / max_size fileios.s read /console print size 
r15 - array pointer random / array pointer fileio.s print/read / console print array

NOT USE
rax, 
rdi,
rsi - file path on read / print array
rdx, 
rcx, 
r8 - array processor i
r9 - seed random
r10, 
r11 -

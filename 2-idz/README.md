...
Так же, в некоторых я переписал обращение к элементу строки - вместо 5 строчек, в которых происходило вычисление смещения, получение адреса и только затем обращением, у меня получилось получилось `movzx   edx, BYTE PTR [r15+rdx]`
